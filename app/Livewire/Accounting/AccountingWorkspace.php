...
class AccountingWorkspace extends Component
{
    ...
    // ── Page resets ────────────────────────────────────────
    public function updatedAccountSearch(): void    { $this->resetPage(); }
    public function updatedAccountTypeFilter(): void { $this->resetPage(); }
    public function updatedJournalSearch(): void    { $this->resetPage(); }
    public function updatedJournalDateFrom(): void  { $this->resetPage(); }
    public function updatedJournalDateTo(): void    { $this->resetPage(); }
    public function updatedLedgerAccountId(): void  { $this->ledgerDateFrom = ''; $this->ledgerDateTo = ''; }
    ...
    public function postSelcomToLedger(): void
    {
        ...
        $selcomAccount = Account::where('name', 'ilike', '%selcom%')->first();
        $cashAccount   = Account::where('name', 'ilike', '%cash%')->orWhere('code', '1000')->first();
    ...
    public function render()
    {
        $accounts = Account::query()
            ->when($this->accountSearch,     fn ($q) => $q->where(function ($q) {
                $q->where('code', 'ilike', "%{$this->accountSearch}%")
                    ->orWhere('name', 'ilike', "%{$this->accountSearch}%");
            }))
        ...
        $journalEntries = JournalEntry::with(['creator', 'lines'])
            ->when($this->journalSearch, fn ($q) => $q->where(function ($q) {
                $q->where('reference', 'ilike', "%{$this->journalSearch}%")
                    ->orWhere('description', 'ilike', "%{$this->journalSearch}%");
            }))
    ...
    private function buildReconciliation(): \Illuminate\Support\Collection
    {
