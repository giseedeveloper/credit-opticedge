...
class RegionalManagerReturnDevicesScreen extends StatefulWidget {
  const RegionalManagerReturnDevicesScreen({super.key});

  @override
  State<RegionalManagerReturnDevicesScreen> createState() => _RegionalManagerReturnDevicesScreenState();
}

class _RegionalManagerReturnDevicesScreenState extends State<RegionalManagerReturnDevicesScreen> {
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      final products = await getRegionalManagerReturnProducts();
      if (!mounted) return;
      setState(() {
        _products = products;
  ...
  Future<void> _onProductChanged(int? productId) async {
    ...
    try {
      final rows = await getRegionalManagerReturnableImeis(productId);
      if (!mounted) return;
      setState(() {
        _imeiRows = rows;
        _loadingImeis = false;
  ...
  Future<void> _submit() async {
    ...
    try {
      final count = await postRegionalManagerReturnDevices(
        productId: pid,
