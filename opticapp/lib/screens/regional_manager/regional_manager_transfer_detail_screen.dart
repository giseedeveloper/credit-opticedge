...
class RegionalManagerTransferDetailScreen extends StatefulWidget {
  const RegionalManagerTransferDetailScreen({super.key});

  @override
  State<RegionalManagerTransferDetailScreen> createState() => _RegionalManagerTransferDetailScreenState();
}

class _RegionalManagerTransferDetailScreenState extends State<RegionalManagerTransferDetailScreen> {
  ...
  Future<void> _load(int id) async {
    ...
    try {
      final data = await getRegionalManagerTransferDetail(id);
      if (!mounted) return;
      setState(() {
        _data = data;
        _loading = false;
  ...
  Future<void> _accept() async {
    ...
    try {
      await acceptRegionalManagerTransfer(id);
      if (!mounted) return;
  ...
  Future<void> _decline() async {
    ...
    setState(() => _acting = true);
    try {
      await declineRegionalManagerTransfer(id);
      if (!mounted) return;
                      ...
                      AdminSectionCard(
                        child: Column(
                          ...
                          children: [
                            ...
                            KeyValueRow(label: 'From admin', value: data['from_admin']?['name']?.toString() ?? '—'),
                            KeyValueRow(label: 'To', value: data['to_regional_manager']?['name']?.toString() ?? '—'),
                            ...
                            if ((data['message'] ?? '').toString().isNotEmpty)
                              KeyValueRow(label: 'Admin note', value: data['message'].toString()),
