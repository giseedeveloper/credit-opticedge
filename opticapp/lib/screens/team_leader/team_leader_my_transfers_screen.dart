...
class TeamLeaderMyTransfersScreen extends StatefulWidget {
  const TeamLeaderMyTransfersScreen({super.key});

  @override
  State<TeamLeaderMyTransfersScreen> createState() => _TeamLeaderMyTransfersScreenState();
}

class _TeamLeaderMyTransfersScreenState extends State<TeamLeaderMyTransfersScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await listTeamLeaderTransfers();
      if (!mounted) return;
      setState(() {
        _list = list;
        _loading = false;
  ...
  Future<void> _tryAccept(Map<String, dynamic> row) async {
    ...
    if (tid == null) return;
    final from = row['from_regional_manager'] as Map<String, dynamic>?;
    final fn = from?['name'] as String? ?? 'regional manager';
    ...
    try {
      await acceptTeamLeaderTransfer(tid);
      if (!mounted) return;
  ...
  Future<void> _tryDecline(Map<String, dynamic> row) async {
    ...
    if (ok != true || !mounted) return;
    try {
      await declineTeamLeaderTransfer(tid);
      if (!mounted) return;
                  ...
                  child: _list.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final from = t['from_regional_manager'] as Map<String, dynamic>?;
                            final fn = from?['name'] as String? ?? 'Regional manager';
                                  ...
                                  children: [
                                    ...
                                    if ((t['message'] as String?)?.isNotEmpty == true)
                                      Padding(
                                        ...
                                        child: Text('Regional manager note: ${t['message']}', style: Theme.of(context).textTheme.bodySmall),
                                      ),
