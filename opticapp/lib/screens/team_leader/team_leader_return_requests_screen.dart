...
import '../../api/team_leader_return_requests_api.dart';
import '../admin/widgets/admin_page_ui.dart';
import '../../theme/app_theme.dart';
import 'team_leader_scaffold.dart';

class TeamLeaderReturnRequestsScreen extends StatefulWidget {
  const TeamLeaderReturnRequestsScreen({super.key});

  @override
  State<TeamLeaderReturnRequestsScreen> createState() => _TeamLeaderReturnRequestsScreenState();
}

class _TeamLeaderReturnRequestsScreenState extends State<TeamLeaderReturnRequestsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        listTeamLeaderReturnRequestsIncoming(),
        listTeamLeaderReturnRequestsOutgoing(),
      ]);
      if (!mounted) return;
      setState(() {
        _incoming = results[0];
  ...
  Future<void> _accept(Map<String, dynamic> row) async {
    ...
    if (rid == null) return;
    try {
      await acceptTeamLeaderReturnIncoming(rid);
      if (!mounted) return;
  ...
  Future<void> _decline(Map<String, dynamic> row) async {
    ...
    if (rid == null) return;
    try {
      await declineTeamLeaderReturnIncoming(rid);
      if (!mounted) return;
  ...
  Future<void> _cancel(Map<String, dynamic> row) async {
    ...
    if (rid == null) return;
    try {
      await cancelTeamLeaderReturnOutgoing(rid);
      if (!mounted) return;
  ...
  Widget _buildList(List<Map<String, dynamic>> list, {required bool incoming}) {
      ...
      return ListView(
        ...
        children: [
          AdminPageEmpty(
            ...
            title: incoming ? 'No return requests from agents.' : 'No return requests sent yet.',
          ),
        ],
      );
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        String title;
        if (incoming) {
          final from = r['from_agent'] as Map<String, dynamic>?;
          title = 'From ${from?['name'] ?? 'Agent'}';
        } else {
          final to = r['to_regional_manager'] as Map<String, dynamic>?;
          title = 'To ${to?['name'] ?? 'Regional manager'}';
        }
        return Container(
