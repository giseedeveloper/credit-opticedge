                ...
                <tbody class="divide-y divide-gray-50 dark:divide-zinc-800">
                    ...
                    @php
                        $channelColor = match($txn->channel) {
                            ...
                            'tigopesa'    => 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
                    ...
                    <tr wire:key="txn-{{ $txn->id }}"
                        ...
                        <td class="px-4 py-3">
                            <p class="font-mono text-xs font-bold text-indigo-600 dark:text-indigo-400">{{ $txn->loan?->loan_number ?? '—' }}</p>
                            @if($txn->description)
        ...
        <div x-show="open"
            ...
            @php
                ...
                $dtColorGrad  = match($dt->channel) {
                    ...
                    'cash'     => 'from-blue-600 to-indigo-700',
                    'tigopesa' => 'from-purple-600 to-violet-700',
                    'halopesa' => 'from-orange-500 to-red-600',
                    'bank'     => 'from-indigo-600 to-blue-700',
                ...
                <div>
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-800 rounded-xl p-4 space-y-3">
                        <div class="flex items-start justify-between">
                            ...
                            <p class="font-mono text-sm font-bold text-indigo-600 dark:text-indigo-400">{{ $dt->loan->loan_number }}</p>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-1">
                            <div class="flex items-center gap-2">
                                ...
                                @if($dt->loan->branch)
                                <span class="text-[10px] text-purple-500 font-semibold">{{ $dt->loan->branch->name }}</span>
                                @endif
                            </div>
                        ...
                        <div class="mt-1 border border-gray-200 dark:border-zinc-700 rounded-lg max-h-44 overflow-y-auto bg-white dark:bg-zinc-900 shadow-lg">
                            @foreach($searchLoans as $ln)
                                <button type="button"
                                    ...
                                    class="w-full text-left px-4 py-2.5 text-sm hover:bg-purple-50 dark:hover:bg-zinc-800 transition-colors @if($paymentLoanId === $ln->id) bg-purple-50 dark:bg-zinc-800 font-semibold text-[#4b0082] @endif">
                                    <span class="font-mono text-purple-600">{{ $ln->loan_number }}</span>
